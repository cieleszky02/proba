...
For a Plan job, `group_plan_jobs_by_seed()` first groups every plan job by its seed element (element_a), so an object with several plan-type neighbours gets one combined job instead of several. `create_plan_callout()` then finds the Floor Plan view already in the project for the connection's level (nearest `Level` by elevation to the first contact's origin) and adds ONE callout on it with `ViewSection.CreateCallout`, sized to the combined bounding box of the seed element and every partner in the group plus a margin, using that Floor Plan's own `ViewFamilyType` (so the callout's family always matches its owner view). If no Floor Plan view exists yet for that level, the whole group is skipped with a note in the output instead of failing the whole run. `tag_connections()` records every pair the combined callout documents (not just one), so each of those type pairs is still individually recognised as already-documented later.

Sheet
-----

`create_connection_sheet()` runs at the end of the same transaction that creates the views, after both loops: it creates a `ViewSheet` (using the first title block type found in the project, or none if the project has none), named "<selected_types_label> - Connections" (deduped like a view name, via `unique_sheet_name`), with an auto-generated sheet number (`unique_sheet_number`, "CT-1", "CT-2", ...), and places every view created in the run on it in a fixed grid (`SHEET_LAYOUT_COLUMNS` × `SHEET_LAYOUT_SPACING_MM`). The grid spacing is arbitrary and views vary a lot in size (a small door detail vs. a combined multi-wall plan callout), so a large view can still overlap its neighbours on the sheet — this needs a manual nudge in that case, not a real collision-aware layout.
...
* [ ] **New:** combined Plan callouts (`group_plan_jobs_by_seed` / `create_plan_callout`): confirm a seed element with several plan-type neighbours produces one callout that actually contains all of them (not clipped), and that its combined-bounding-box sizing doesn't produce a degenerate or absurdly large callout when a partner element is far away.
