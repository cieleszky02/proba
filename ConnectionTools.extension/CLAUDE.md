...
1. The user filters by category, then multi-selects one or more family types actually placed in the model (`types_in_use` — a loaded-but-unplaced type isn't offered, e.g. every window type, or a specific wall type). If more than one instance of those types exists, `choose_seed_elements()` then asks whether to use all of them, pick specific ones by clicking in the model (candidates highlighted blue beforehand, same as any Revit selection), or pick from a list.
2. Every one of those chosen instances is checked against every other configured component. Connections are deduped and documented by TYPE PAIR, not by element instance: once one instance of "Door Type X" has a documented connection to "Wall Type Y", every other instance of that same pair is skipped, since it would produce an identical, redundant view (see "already documented" below).
...
5. Result: each candidate gets a `Contact` object with `kind` (side/top/bottom/overlap/host), `origin` (middle of the contact region), `joint_dir`, and `length`.

`joint_dir` is NOT derived from the face/overlap geometry above — `detect_contact()` overwrites whatever the face/overlap/host path computed with `_center_to_center_dir()`: the horizontal direction perpendicular to the line between the two elements' bounding-box centres. This means the cut plane always contains that centre-to-centre axis and both elements land cleanly in the section, regardless of how messy either element's actual geometry is. This replaced a PCA-over-contact-points direction that worked for simple planar elements (walls, floors) but landed at an arbitrary, non-grid-aligned angle for a stair (many small tread/riser faces) or was otherwise unpredictable — seen as a section line cutting diagonally across the whole floor plan instead of running parallel to a wall or grid line. `origin`, `kind`, and `length` still come from the real contact geometry (face overlap or solid intersection) — only the direction was unreliable.
...
* [x] `joint_dir` was computed via PCA over face/overlap contact points, which worked for simple planar elements but produced an arbitrary, non-grid-aligned angle for a stair (confirmed from a real run — a section line cutting diagonally across the whole floor plan instead of running parallel to a wall or grid line). Replaced with `_center_to_center_dir()`: perpendicular to the straight line between the two elements' bounding-box centres, so the cut plane always contains that axis regardless of geometry complexity. Needs a real retest on the same stair case, and on a few ordinary wall/floor cases, to confirm this reads at least as well as the old PCA direction for the cases that were already working (columns, doors) and actually fixes the stair case.
...
* [ ] **New:** `choose_seed_elements()` / `_pick_instances_in_model()`: confirm `PickObjects` (plural, multi-pick with a Finish button) behaves as expected, that candidates actually highlight blue beforehand via `SetElementIds`, and that the previous selection is correctly restored afterward.
